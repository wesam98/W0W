...
```bash
    db shell am start -n com.mobilehackinglab.documentviewer/.MainActivity \
...
![image.png](/img/image61.png)

- Then tested path traversal — encoded each `/` as `%2F` and pointed the URL into the app's private `files/` directory: trigger activity via ADB again
...
**and It worked** `testtt.pdf` written outside the intended Downloads directory 

![image.png](/img/image62.png)

---

1. **The second interesting function was`loadProLibrary()`** 
    ...
    class PDFRequestHandler(BaseHTTPRequestHandler):
        def do_GET(self):
            ...
            if self.path == "/test.pdf":
                ...
                self.end_headers()
                self.wfile.write(b"PDF ServerYou can access the PDF file here.")
